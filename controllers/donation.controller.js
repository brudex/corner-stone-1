...
Controller.settlementSummary = async (req, res) => {
  ...
  let sql = `SELECT cd.churchId,c.name ,sum(cd.amount) 'amountPaid',sum(cd.amount2) 'settleAmount', sum(cd.profit) 'profit'  , cd.settlementStatus from churchdonation cd LEFT JOIN church c on c.id=cd.churchId where cd.settlementStatus='${settlementStatus}' AND cd.createdAt BETWEEN '${startDate}' and '${endDate}' GROUP BY cd.churchId `;
  if (req.user.isSuperAdmin) {
    if (req.query.churchId) {
        churchId = req.query.churchId;
         sql = `SELECT cd.churchId,c.name ,sum(cd.amount) 'amountPaid',sum(cd.amount2) 'settleAmount',sum(cd.profit) 'profit'  , cd.settlementStatus from churchdonation cd LEFT JOIN church c on c.id=cd.churchId where cd.settlementStatus='${settlementStatus}' AND cd.createdAt BETWEEN '${startDate}' and '${endDate}' and churchId=${churchId} GROUP BY cd.churchId `;
    }
  }else{
    churchId = req.user.churchId;
    sql = `SELECT cd.churchId,c.name ,sum(cd.amount) 'amountPaid',sum(cd.amount2) 'settleAmount',sum(cd.profit) 'profit' , cd.settlementStatus from churchdonation cd LEFT JOIN church c on c.id=cd.churchId where cd.settlementStatus='${settlementStatus}' AND cd.createdAt BETWEEN '${startDate}' and '${endDate}' and churchId=${churchId} GROUP BY cd.churchId `;
